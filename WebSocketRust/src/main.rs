use actix::{
    Actor, ActorContext, Addr, AsyncContext, Handler, Message, Recipient, StreamHandler,
    WrapFuture, clock::Instant, dev::Stream, fut,
};
...
//This is creating an actor thing.
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || App::new().app_data(lobby_data.clone()).service(index))
        .bind(("127.0.0.1", 8080))?
        .run()
        .await
}
